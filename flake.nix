  ...
  outputs = { self, nixpkgs, rust-overlay, crane }@inputs: let
    ...
    workspaceCrates = {
      ...
      cli = {
        name = "metalps-cli";
        binary = "metalps";
        description = "GPU process monitor (like ps, but for the GPU)";
      };

      # GPU load generator for integration testing
      gpu-load = {
        name = "metalps-gpu-load";
        binary = "metalps-gpu-load";
        description = "Metal compute workload generator for testing";
      };
  ...
  in {
    ...
    devShells = forAllSystems (system: {
      default = (pkgsFor system).mkShell {
        ...
        shellHook = ''
          echo "metalps - GPU process monitor development environment"
          echo ""
